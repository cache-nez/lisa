...
class UnificationTests extends AnyFunSuite {
  ...
  test("unification 2") {
    checkUnifiesAs(a, a, emptyContext)
    checkDoesNotUnify(a, b)
    checkDoesNotUnify(a, sa)
    checkUnifiesAs(a /\ b, a /\ b, emptyContext)

    checkUnifiesAs(sa, a, emptyContext + AssignedPredicate(sa, a))
    checkUnifiesAs(sa, sa, emptyContext + AssignedPredicate(sa, sa))
    checkUnifiesAs(sa /\ b, a /\ b, emptyContext + AssignedPredicate(sa, a))
    checkUnifiesAs(sa /\ sb, a /\ b, emptyContext + AssignedPredicate(sa, a) + AssignedPredicate(sb, b))
    checkUnifiesAs(sa /\ b, sb /\ b, emptyContext + AssignedPredicate(sa, sb))
    checkUnifiesAs(sa /\ sb, (a ==> b) /\ (c \/ a), emptyContext + AssignedPredicate(sa, a ==> b) + AssignedPredicate(sb, c \/ a))
    checkUnifiesAs(sa /\ sa, b /\ b, emptyContext + AssignedPredicate(sa, b))
    checkUnifiesAs(sa /\ sa, (a \/ b) /\ (b \/ a), emptyContext + AssignedPredicate(sa, a \/ b))
    checkDoesNotUnify(sa /\ sa, a /\ b)

    checkUnifiesAs(sa, a /\ b, emptyContext + AssignedPredicate(sa, b /\ a), emptyContext + AssignedPredicate(sa, b /\ a))
    checkUnifiesAs(sa, a, emptyContext + AssignedPredicate(sa, a), emptyContext + AssignedPredicate(sa, a))
    checkDoesNotUnify(sa, a, emptyContext + AssignedPredicate(sa, b))
    checkDoesNotUnify(sa, a, emptyContext + AssignedPredicate(sb, a))

    checkUnifiesAs(p1(t), p1(t), emptyContext)
    checkUnifiesAs(p1(st), p1(u), emptyContext + AssignedFunction(st, u))
    checkUnifiesAs(sp1(t), p1(t), emptyContext + AssignedPredicate(sp1, LambdaPredicate(x => p1(x))))
    checkUnifiesAs(sp1(t), p1(t) /\ p1(u), emptyContext + AssignedPredicate(sp1, LambdaPredicate(x => p1(x) /\ p1(u))))

    checkUnifiesAs(sg1(a), b, emptyContext + AssignedConnector(sg1, LambdaConnector(_ => b)))
    checkDoesNotUnify(sg1(sa), a)
    checkUnifiesAs(sg1(sa), b, emptyContext + AssignedConnector(sg1, LambdaConnector(x => x)), emptyContext + AssignedConnector(sg1, LambdaConnector(x => x)) + AssignedPredicate(sa, b))
    checkUnifiesAs(sg1(sa), b, emptyContext + AssignedPredicate(sa, b), emptyContext + AssignedPredicate(sa, b) + AssignedConnector(sg1, LambdaConnector(x => x)))
    ...
    checkUnifiesAs(exists(x, a), exists(x, a), emptyContext + (x -> x))
    checkUnifiesAs(exists(x, a), exists(y, a), emptyContext + (x -> y))
    checkUnifiesAs(exists(x, p1(x)), exists(y, p1(y)), emptyContext + (x -> y))
    checkDoesNotUnify(exists(x, p1(x)), exists(y, p1(z)))
    checkUnifiesAs(p1(x), p1(x), emptyContext + (x -> x))
    checkUnifiesAs(p1(x), p1(y), emptyContext + (x -> y))
    checkUnifiesAs(p1(x), p1(y), emptyContext + (x -> y), emptyContext + (x -> y))
    checkDoesNotUnify(p1(x), p1(y), emptyContext + (x -> z))

    checkUnifiesAs(exists(x, sp1(x)), exists(y, p1(y) /\ a), emptyContext + (x -> y) + AssignedPredicate(sp1, LambdaPredicate(v => p1(v) /\ a)))
    checkUnifiesAs(exists(x, sa), exists(x, p1(t)), emptyContext + (x -> x) + AssignedPredicate(sa, p1(t)))
    ...
    checkUnifiesAs(p1(st) /\ p1(su), p1(x) /\ p1(x), emptyContext + AssignedFunction(st, x) + AssignedFunction(su, x))
    checkDoesNotUnify(p1(st) /\ p1(st), p1(x) /\ p1(y))
    checkDoesNotUnify(p1(st) /\ p1(st), p1(su) /\ p1(sv))
    checkUnifiesAs(p1(st) /\ p1(st), p1(su) /\ p1(su), emptyContext + AssignedFunction(st, su))

    checkUnifiesAs(exists(x, exists(y, p1(x) /\ p1(y))), exists(y, exists(x, p1(y) /\ p1(x))), emptyContext + (x -> y) + (y -> x))
  }
}
